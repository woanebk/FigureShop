...
export default function AddUserScreen({route, navigation}) {
  ...
  const [firstRun,setFirstRun]=useState(0); // Lần chạy đầu tiên useEffect sẽ gọi get User để đăng kí listenr dữ liệu (Những lần useEffect sau sẽ bỏ qua- tránh lỗi infinite loop)
  ...
  const [UserName, setUserName] = useState(''); // ten User
  ...
  const uploadImage = async (uri)=>{
    ...
    const ref = firebaseApp.storage().ref().child('images/User/' + UserName +'.jpg');
  ...
  const GetUser = async ()=>{
    await firebaseApp.database().ref('User/' + UserID).on('value', (snapshot)=>{
      console.log('on')
      if( snapshot.exists())
      {
        setuser(snapshot.val().TenUser)
  ...
  const AddUser = (ten_User, url_hinh_anh)=>{
  ...
  const UpdateUser = (ten_User, url_hinh_anh)=>{
    firebaseApp.database().ref('User/' + UserID).update({
      TenUser: ten_User,
      HinhAnh: url_hinh_anh,
    })
  }
  ...
  const Submit = ()=>{
    ...
    if(!readonly) //================ ADD
    {
      ...
      uploadImage(image)
        .then(()=>{ //sau khi up load thanh cong goi api lay download url cua hinh anh
          return firebaseApp.storage().ref().child('images/User/' + UserName +'.jpg').getDownloadURL();
        })
    ...
    else //=========================EDIT
    {
      uploadImage(image)
        ...
        .then((url)=>{ //gan download url vao bien state imageURL
          setImageURL(url);
          UpdateUser(UserName, url);
        })
        .then(()=>{
          setIsLoading(false);
          setEditing(false)
          alert('Sửa User Thành Công');
        })
        .catch((error)=>{
          setIsLoading(false);
          alert(error);
      ...
      <View style={styles.topdock}></View>
      {
        ...
        <ScrollView style={styles.infoWrapper}>
          ...
          <ActionInput title = 'Tên Loại Sản Phẩm' ionIconName='ios-logo-android'
            placeholder='Nhập Tên Loại Sản Phẩm' value={UserName} 
            editable={editing} onChangeText={(text)=>{setUserName(text)}}
          ></ActionInput>
